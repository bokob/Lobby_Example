...
public class NetworkGameManager : NetworkBehaviour
{
    ...
    public int GetPlayerSkinFromIndex(int _playerIndex) // 플레이어 스킨 변경 (쓸데 없음)
    {
        return playerDataNetworkList[_playerIndex].skinIndex;
    }

    public void ChangePlayerSkin(int skinIndex) // 플레이어 스킨 변경, 서버에 RPC를 호출하여 변경 (쓸데 없음)
    {
        ChangePlayerSkinServerRpc(skinIndex);
    }

    [ServerRpc(RequireOwnership = false)]
    void ChangePlayerSkinServerRpc(int skinIndex, ServerRpcParams rpcParams = default) // (쓸데 없음)
    {
    ...
    private void Network_OnClientDisconnectCallback(ulong clientId) // 클라이언트 연결이 해제되었을 때 호출
    {
        ...
        {
            // failed to connect
            FindObjectOfType<LobbyBrowseUI>().ConnectionFailed(); // 연결 오류 메시지 UI에 전달
        }
        else if (SceneManager.GetSceneByName("MULTIPLAYER_LOBBY_JOINED_SCENE") == SceneManager.GetActiveScene()) // 로비(게임 방) 씬
        {
            // inside a lobby;
    ...
    void Network_ConnectionApprovalCallback(NetworkManager.ConnectionApprovalRequest connectionApprovalRequest, NetworkManager.ConnectionApprovalResponse connectionApprovalResponse)
    {
        ...
        if (NetworkManager.Singleton.ConnectedClientsIds.Count >= MAX_PLAYERS) // 가득찬 방이면
        {
            connectionApprovalResponse.Approved = false;
    ...
    {
        LobbyManager.instance.DeleteLobby(); // 게임 중인 방은 로비 목록에 안뜨게 하니까 그냥 지워버린다.
